...
net-spec "$1".net 10 30 30 30 30 30 1 / ih0=2:0.5:0.5 bh0=5:0.5 h0h1=2:0.5 bh1=5:0.5 h1h2=2:0.5 bh2=5:0.5 h2h3=2:0.5 bh3=5:0.5 h3h4=2:0.5 bh4=5:0.5 h4o=x2:0.5 bo=100 
...
data-spec "$1".net 10 1 2 / "$1"_train.csv@1: . "$1"_test.csv@1: .
...
mc-spec "$1".net repeat 10 sample-noise heatbath hybrid 50:10 0.3
...
mc-spec "$1".net repeat 10 sample-sigmas heatbath 0.95 hybrid 50:10 0.13 negate
# run 50,000 iterations
net-mc "$1".net 200000

# export chains of all hyperparameter to check convergence
net-plt t h1h2h3h4h5h6h7h8h9h10h11 "$1".net > order_chain_"$1".txt
# export the hyperparameters associated with each input neuron
net-display -h "$1".net > order_struct_"$1".txt
# use the last 25,000 iterations for prediction
net-pred tmp "$1".net 160000: > order_result_"$1".txt

# trim the result file so that the predictions could be readable in R
sed -i 1,5d order_result_"$1".txt
sed -i "$(($(wc -l < order_result_"$1".txt)-3)),\$d" order_result_"$1".txt
