...
net-spec "$1".net 16 60 60 60 60 60 1 / ih0=2:0.5:0.5 bh0=5:0.5 h0h1=2:0.5 bh1=5:0.5 h1h2=2:0.5 bh2=5:0.5 h2h3=2:0.5 bh3=5:0.5 h3h4=2:0.5 bh4=5:0.5 h4o=x2:0.5 bo=100 
...
data-spec "$1".net 16 1 2 / "$1"_train.csv@1: . "$1"_test.csv@1: .
...
mc-spec "$1".net repeat 10 sample-noise heatbath hybrid 50:10 0.2
...
mc-spec "$1".net repeat 10 sample-sigmas heatbath 0.95 hybrid 50:10 0.08 negate
# run 50,000 iterations
net-mc "$1".net 100000

# use the last 25,000 iterations for prediction
net-pred tmp "$1".net 80000: > soil_disease_result_"$1".txt

# trim the result file so that the predictions could be readable in R
sed -i 1,5d soil_disease_result_"$1".txt
sed -i "$(($(wc -l < soil_disease_result_"$1".txt)-3)),\$d" soil_disease_result_"$1".txt
