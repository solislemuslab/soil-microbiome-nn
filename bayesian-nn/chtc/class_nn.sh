...
net-spec "$1".net 108 300 300 300 300 1 / ih0=2:0.5:0.5 bh0=5:0.5 h0h1=2:0.5 bh1=5:0.5 h1h2=2:0.5 bh2=5:0.5 h2h3=2:0.5 bh3=5:0.5 h3o=x2:0.5 bo=100
...
data-spec "$1".net 108 1 2 / "$1"_train.csv@1: . "$1"_test.csv@1: .
...
mc-spec "$1".net repeat 10 sample-noise heatbath hybrid 40:8 0.12
...
mc-spec "$1".net repeat 10 sample-sigmas heatbath 0.95 hybrid 5:1 0.01 negate
# run 50,000 iterations
net-mc "$1".net 20000

# export chains of all hyperparameter to check convergence
net-plt t h1h2h3h4h5h6h7h8h9 "$1".net > class_chain_"$1".txt
...
net-pred tmp "$1".net 15000: > class_result_"$1".txt
