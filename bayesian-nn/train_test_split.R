...
split <- function(full_data, filename, location) {
  ...
  fullpath_train = paste(location, "/train-test-split/", name, "_train.csv", sep = "")
  fullpath_test = paste(location, "/train-test-split/", name, "_test.csv", sep = "")
...
for (i in 1:length(levels)){
  load("all_otu_original", -1, levels[i])
...
for (i in 1:4){
  load("all_otu_augmented", -1, levels[i])
}
