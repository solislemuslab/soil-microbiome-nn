...
all_otu_split <- function(level, aug) {
  path = paste("../processed-data/otu_data/all_otu/", aug, "/"
               , level, "/full-data", sep = "")
  file_name = dir(path)
  path_name = dir(path, full.names = TRUE)
  all_files = lapply(path_name, read.csv, header = FALSE)
  ...
  for (i in 1:num) {
    split_all_otu(all_files[[i]], level, file_name[i], aug)
  }
}

split_all_otu <- function(full_data, level, filename, aug){
  name = substr(filename, 1, nchar(filename) - 4)
  split_data <- train_test_split(
    full_data, 
    prop = 0.8,
  ...
  fullpath_train = paste("../processed-data/otu_data/all_otu/", aug, "/", level
                         , "/train-test-split/", name, "_train.csv", sep = "")
  fullpath_test = paste("../processed-data/otu_data/all_otu/", aug, "/", level
                        , "/train-test-split/", name, "_test.csv", sep = "")
...
levels <- cbind("Phylum", "Class", "Order", "Family")
for (i in 1:length(levels)){
  all_otu_split(levels[i], "augumented")
}
for (i in 1:length(levels)){
  all_otu_split(levels[i], "non-augumented")
}
