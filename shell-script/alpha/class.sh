...
net-plt t h1h2h3h4h5h6h7h8h9h10h11 "$1".net > class_chain_"$1".txt
# export the hyperparameters associated with each input neuron
net-display -h "$1".net > class_struct_"$1".txt
# use the last 25,000 iterations for prediction
net-pred tmp "$1".net 160000: > class_result_"$1".txt

# trim the result file so that the predictions could be readable in R
sed -i 1,5d class_result_"$1".txt
sed -i "$(($(wc -l < class_result_"$1".txt)-3)),\$d" class_result_"$1".txt
